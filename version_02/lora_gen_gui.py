...
# JSON 转换为 JSONL
def convert_json_to_jsonl(input_json_path, output_jsonl_path, pairs_per_line=1):
    try:
        ...
        with open(output_jsonl_path, 'w', encoding='utf-8') as f:
            pairs_per_line = max(1, pairs_per_line)
            for i in range(0, len(input_data), pairs_per_line):
                messages = []
...
# GUI 主程序
def create_gui():
    global root, api_key_var, base_url_var, model_var, max_tokens_var, jsonl_pairs_var
    root = tk.Tk()
    ...
    ttk.Label(button_frame_right, text="数据对数:").pack(side=tk.LEFT, padx=2)
    jsonl_pairs_var = tk.StringVar(value="1")
    jsonl_pairs_entry = ttk.Entry(button_frame_right, textvariable=jsonl_pairs_var, width=5)
    jsonl_pairs_entry.pack(side=tk.LEFT, padx=5)
    ...
    def convert_and_display(json_widget, jsonl_widget):
        ...
        try:
            pairs_per_line = int(jsonl_pairs_var.get())
        except ValueError:
            pairs_per_line = 1

        if os.path.exists(input_file):
            if convert_json_to_jsonl(input_file, output_file, pairs_per_line):
